...
class myLinear(nn.Linear):
    def __init__(self, J_interaction, bias):
        super(myLinear, self).__init__(
            J_interaction.shape[1], J_interaction.shape[0], bias
        )
...
class bp_nn_2(bp_nn.bp_nn):
    ...
    def forward_m(self, sample):
        ...
        prob_minus = torch.log(1.0 - prob_marginals)
        # print(prob_plus)
        # print(prob_minus)
    ...
    def _log_prob(self, sample, x_hat_p, x_hat_m):
        mask = (sample + 1) / 2
        log_prob = torch.log(x_hat_p + self.epsilon) * mask + torch.log(
            x_hat_m + self.epsilon
        ) * (1 - mask)
    ...
    def train(
        ...
        for step in range(0, max_step + 1):
            ...
            loss_reinforce = (
                p_sample * ((loss - (loss * p_sample).sum()) * log_prob)
            ).sum()
            ...
            print(
                "\r {beta:.2f} {step} fe: {0:.3f} +- {1:.5f}, E: {E:.3f}, S: {S:.3f}, M: {2:.3}, #s: {n_sample} B1: {B1:.3f}".format(
                    self.F,
                    self.F_std,
                    self.M,
                    n_sample=len(sample),
