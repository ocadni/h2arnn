...
class SK_krsb(nn.Module):
    ...
    def forward(self, m):
        ...
        if len(m.shape) > 1:
            m_i = m[:, 0]
            m_n_i = m[:, 1:]
            print(m_n_i.shape, self.weight_p.shape, self.bias_p.shape)
            ...
            res += res_p.sum(dim=1) + res_m.sum(dim=1)

        res += self.bias_0 + self.weight_0 * m_i
...
class h2arnn(ANN):
    def __init__(
        ...
        for n_i in range(model.N):
            dep = J_interact[0:n_i, n_i:]
            mask1.append(torch.argwhere(torch.sum(dep, dim=0)).squeeze(1))
            N_i = len(mask1[-1])-1 if len(mask1[-1]) > 0 else 0
            net.append(single_net(model, n_i, N_i=N_i, device=device,
        ...
        self.J_interact = J_interact.to(dtype=torch.bool, device=device)
        self.input_mask = torch.argwhere(
            torch.sum(J_interact, dim=1)).squeeze(1)
        self.mask1 = mask1
    ...
    def first_l(self, x, n_i):
        ...
        dep = self.J[0:n_i, n_i:]
        mask_n_i = self.input_mask
        print(f"n_i:{n_i}", mask_n_i[:n_i], self.mask1[n_i],
              dep.shape)
        J_n_i = dep[mask_n_i[:n_i]][:self.mask1[n_i]]
        print(f"n_i:{n_i}", mask_n_i[:n_i], self.mask1[n_i], J_n_i)
        if len(J_n_i.shape) == 1 and n_i > 0:
            J_n_i = J_n_i.unsqueeze(1)
        print(n_i, J_n_i.shape, x.shape)
        return F.linear(x, J_n_i)
