...
class BP_solver:
    def __init__(self, model_):
        ...
        self.N = model_.N
        if torch.is_tensor(self.J):
            self.J = self.J.cpu().detach().numpy()
        if torch.is_tensor(self.J_interaction):
            self.J_interaction = self.J_interaction.cpu().detach().numpy()
        if torch.is_tensor(self.H):
            self.H = self.H.cpu().detach().numpy()

        # matrix of message (represent +1 probability)
        self.M = self.J_interaction.copy()
        ...
        for i in range(len(neighs)):
            ...
            for j in neighs[i][neighs[i] != -2]:
                if j > i:
                    Z_ij_temp = 0
                    Z_ij_temp += (
                        factor_ij(i, j, 1, 1, beta) * Z_ij_p[i][j] * Z_ij_p[j][i]
                    )
                    Z_ij_temp += (
                        factor_ij(i, j, 1, -1, beta) * Z_ij_p[i][j] * Z_ij_m[j][i]
                    )
                    Z_ij_temp += (
                        factor_ij(i, j, -1, 1, beta) * Z_ij_m[i][j] * Z_ij_p[j][i]
                    )
                    Z_ij_temp += (
                        factor_ij(i, j, -1, -1, beta) * Z_ij_m[i][j] * Z_ij_m[j][i]
                    )
                    F_ij += np.log(Z_ij_temp)
                    # print(Z_ij_temp,  factor_ij(i, j, 1, 1) * Z_ij_p[i][j] * Z_ij_p[j][i])
        # print(Z_ij)
        self.Z_ij = np.exp(F_ij)
        self.F = -1.0 / beta * (F_ij + F_i) * (1.0 / self.N)
        if print_:
    ...
    def converge(self, beta, error=1e-10, max_iter=1000, rand_init=True, val_rand=0.1):
        if rand_init:
        ...
        while err_temp > error and iter_ < max_iter:
            ...
            iter_ += 1
            print(
                "\r iter:{1},  err: {0:.3f} free_energy {2:.2f}".format(
                    err_temp, iter_, self.free_energy(beta, print_=False)
                ),
                end="",
            )
        print("\r", end="")
        ...
        fe = self.F
        print(
            "fe: {0:.3f}, ener: {1:.3f}, M: {2:.3f}, iter {iter_}".format(
                fe, E, M, iter_=iter_
            )
        )
