...
from python_lib import bp_solver


def bp_sol(model, betas, error=1e-6, val_rand=0.1, max_iter=1000):
    fe_bp = []
    ener_bp = []
    m_bp = []
    m_i_bp = []
...
def train_nn_anneal(
    ...
    max_step=10,
    batch_size=1000,
    batch_size_stats=None,
    std_fe_limit=1e-5,
    opt="adam",
):
    if batch_size_stats == None:
...
def nn_sol(
    ...
    m_i_nn = []
    c_i_nn = []
    net = nn_use(model.N, model, bias, z2=z2, x_hat_clip=x_hat_clip)
    ...
    for beta in betas:
        ...
        # print("\r", end="")
        print(
            "\rfe: {0:.3f} std_fe: {1:.2E} M: {2:.3f} S: {3:.3f} E: {4:.3f}".format(
                F,
                F_std,
                M,
                S,
...
def plot_quantity(label, res_ex, others, init_=0):
    ...
    for other in others:
        plt.plot(
            other["betas"][init_:],
            (other[label][init_:] - res_ex[label][init_:]),
            "-o",
            label=other["name"],
        )
    plt.legend()
    ...
    for other in others:
        plt.plot(
            other["betas"][init_:],
            100
            * abs(other[label][init_:] - res_ex[label][init_:])
            / abs(res_ex[label][init_:] + 1e-6),
            "-o",
            label=other["name"],
        )
...
def plot_quantity_sum(label, res_ex, others, init_=0):
    ...
    ax0.plot(
        res_ex["betas"][init_:],
        abs(res_ex[label][init_:]).sum(axis=1) / len(res_ex[label]),
        "o",
        label="exact",
    )
    for other in others:
        ax0.plot(
            other["betas"][init_:],
            abs(other[label][init_:]).sum(axis=1) / len(other[label]),
            "o",
            label=other["name"],
        )
    ax0.legend()
    ...
    for other in others:
        ax1.plot(
            other["betas"][init_:],
            abs(abs(other[label][init_:]) -
                abs(res_ex[label][init_:])).sum(axis=1)
            / len(other[label]),
            "o",
            label=other["name"],
        )
    ...
    for other in others:
        ax2.plot(
            other["betas"][init_:],
            100
            * abs(abs(other[label][init_:]) - abs(res_ex[label][init_:])).sum(axis=1)
            / len(other[label])
            / (res_ex[label][init_:].sum(axis=1) + 1e-10),
            "o",
            label=other["name"],
        )
...
def plot_all_corr(
    betas, corr_ex, others, J_interaction, init_=0, num_col=3, label="C_ij"
):
