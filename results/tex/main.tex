...
7. Several interesting future can be derived that this derived architecture shared with common used architecture:
      ...
      In this paper, we explore the connections between Statistical Physics and Machine Learning, specifically in the context of generative models. Both fields deal with high-dimensional input functions, and in recent years, the use of neural network-based frameworks in Machine Learning has achieved remarkable success in a variety of domains. The overlap between these fields has led to important and fundamental results in both domains, with applications of Machine Learning techniques in Statistical Physics spanning a wide range of areas, including quantum mechanics, classical statistical physics, chemical physics, and complex systems.
...
\begin{eqnarray}
P\left(x_{i}|\mathbf{x}_{<i}\right) = \frac{P\left(x_1,x_2,\dots x_{i}\right)}{P\left(x_1,x_2,\dots x_{i-1}\right)}  = \frac{\sum_{x_{i+1}\dots x_{N}}P\left(\mathbf{x}\right)}{\sum_{x_{i}\dots x_{N}}P\left(\mathbf{x}\right)} & = & \frac{\sum_{x_{i+1}\dots x_{N}}e^{-\beta H}}{\sum_{x_{i}\dots x_{N}}e^{-\beta H}} = \frac{f\left(x_{i},\mathbf{x}_{<i}\right)}{\sum_{x_{i}}f\left(x_{i},\mathbf{x}_{<i}\right)}
\label{eq:chain}
\end{eqnarray}
